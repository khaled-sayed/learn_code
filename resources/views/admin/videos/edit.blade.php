                        ...
                        <form method="post" action="{{ route('videos.update', $video) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-name" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Title') }}" value="{{$video->title }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('link') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="link" id="input-email" class="form-control form-control-alternative{{ $errors->has('link') ? ' is-invalid' : '' }}" placeholder="{{ __('Link') }}" value="{{ $video->link }}" required>
                                ...
                                <div class="form-group{{ $errors->has('course_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="course_id" class="custom-select" id="course" required>
                                        ...
                                        <option <?php if($course->id == $video->course->id) { echo"selected"; } ?> value="{{$course->id}}">{{Str::limit($course->title, 50)}}</option>
                                        @endforeach
                                    </select>
